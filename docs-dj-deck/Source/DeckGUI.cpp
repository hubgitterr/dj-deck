...

{
    ...
    //changing volume and speed sliders into Rotary sliders
    ...
    trackName.setTextToShowWhenEmpty("Not loaded", juce::Colours::pink);
...
void DeckGUI::paint (juce::Graphics& g)
{
    ...
    */
    ...
    // here are my parts of code
    g.fillAll(juce::Colours::cornflowerblue);   // clear the background
    ...
    g.setFont (14.0f);
    g.drawText ("DJ Rubs Deck", getLocalBounds(),
...
void DeckGUI::resized()
{
    ...
    loopButton.setBounds((getWidth() / 7)*2, 0, getWidth() / 7, rowH);
    trackName.setBounds((getWidth() / 7) * 3.5, 0, (getWidth() / 7) * 3, rowH);
...
void DeckGUI::buttonClicked(juce::Button* button) 
{
    ...
    { //If button is clicked, start the audio file
        DBG("Play button is clicked");
        player->start();
    }
    if (button == &stopButton) 
    { //If button is clicked, stops the audio file
        DBG("Stop button is clicked");
        player->stop();
    }
    if (button == &loadButton) 
    { //If button is clicked, allows user to select file to be loaded into the deck
...
{ //Timer loops every 100ms, which checks if the "loop" button is on or off and also updates the wave form display current position
    if (std::to_string(loopButton.getToggleState()) == "1") 
    { //If loop button is checked
        if (player->getPositionRelative() >= 1) 
        {// and the position more than 1 meaning it is the end of the audio file
...
{ //function for loading and checking playlist
